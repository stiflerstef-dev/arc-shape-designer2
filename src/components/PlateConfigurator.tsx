        ...
        <div className="grid lg:grid-cols-2 gap-10 lg:gap-14 items-start">
          ...
          <div className="sticky top-0 z-30 -mx-6 md:-mx-12 lg:mx-0 lg:order-2 lg:sticky lg:top-4 lg:self-start bg-background lg:bg-transparent pt-2 pb-3 lg:p-0 border-b border-border lg:border-b-0">
            <div className="relative bg-canvas overflow-hidden border-y border-border lg:border lg:rounded-sm">
              ...
              {/* ─── Detail: 3D worm's-eye view of the arch with mounted spot ─── */}
              {hasLight && archType !== "gothic" && (
                <div className="absolute bottom-5 right-5 bg-card border border-border rounded-sm shadow-sm p-3" style={{ width: 190 }}>
                  <p className="text-[9px] tracking-[0.2em] uppercase text-muted-foreground mb-2 text-center">Onderaanzicht boog · Spot</p>
                  <svg width={170} height={140} viewBox="0 0 170 140" className="mx-auto block">
                    <defs>
                      <linearGradient id="ceilGrad3D" x1="0" y1="0" x2="0" y2="1">
                        <stop offset="0%" stopColor={COL.nicheCeiling} stopOpacity={0.55} />
                        <stop offset="100%" stopColor={COL.nicheCeiling} stopOpacity={1} />
                      </linearGradient>
                      <linearGradient id="wallGradL" x1="0" y1="0" x2="1" y2="0">
                        <stop offset="0%" stopColor={COL.nicheWall} stopOpacity={0.6} />
                        <stop offset="100%" stopColor={COL.nicheWall} stopOpacity={1} />
                      </linearGradient>
                      <linearGradient id="wallGradR" x1="1" y1="0" x2="0" y2="0">
                        <stop offset="0%" stopColor={COL.nicheWall} stopOpacity={0.6} />
                        <stop offset="100%" stopColor={COL.nicheWall} stopOpacity={1} />
                      </linearGradient>
                      <radialGradient id="spotGlow3D" cx="50%" cy="50%" r="50%">
                        <stop offset="0%" stopColor="hsl(45 100% 92%)" stopOpacity={0.95} />
                        <stop offset="60%" stopColor="hsl(45 95% 75%)" stopOpacity={0.55} />
                        <stop offset="100%" stopColor="hsl(45 90% 65%)" stopOpacity={0} />
                      </radialGradient>
                    </defs>
                    {(() => {
                      // Front opening (close to viewer) — wide arch at bottom
                      const fx1 = 18, fx2 = 152, fyBase = 120;
                      const fArchTop = 60; // height of front opening's straight wall portion top
                      const fr = (fx2 - fx1) / 2; // arch radius for front opening
                      const fcx = (fx1 + fx2) / 2;
                      // Back wall (smaller, receded) — narrower arch up & back
                      const bx1 = 58, bx2 = 112, byBase = 70;
                      const bArchTop = 40;
                      const br = (bx2 - bx1) / 2;
                      const bcx = (bx1 + bx2) / 2;

                      // Front opening path (arch silhouette)
                      const frontPath = `M ${fx1} ${fyBase} L ${fx1} ${fArchTop + fr} A ${fr} ${fr} 0 0 1 ${fx2} ${fArchTop + fr} L ${fx2} ${fyBase} Z`;
                      // Back arch path (recessed)
                      const backPath = `M ${bx1} ${byBase} L ${bx1} ${bArchTop + br} A ${br} ${br} 0 0 1 ${bx2} ${bArchTop + br} L ${bx2} ${byBase} Z`;

                      // Ceiling (between front arch top edge & back arch top edge) — perspective panel
                      // Sample arc points
                      const arcPoints = (cx: number, cy: number, r: number, steps: number) => {
                        const pts: string[] = [];
                        for (let i = 0; i <= steps; i++) {
                          const t = Math.PI + (i / steps) * Math.PI; // from left to right over top
                          pts.push(`${cx + r * Math.cos(t)},${cy + r * Math.sin(t)}`);
                        }
                        return pts;
                      };
                      const frontArc = arcPoints(fcx, fArchTop + fr, fr, 14);
                      const backArc = arcPoints(bcx, bArchTop + br, br, 14).reverse();
                      const ceilPath3D = `M ${frontArc.join(" L ")} L ${backArc.join(" L ")} Z`;

                      // Spot position — centered in ceiling, viewed from below at an angle → ellipse
                      const spotCx = 85;
                      const spotCy = 58; // slightly back from front
                      return (
                        <g>
                          {/* Cabinet front face hint behind */}
                          <rect x={6} y={108} width={158} height={28} fill={COL.front} stroke={COL.frontStroke} strokeWidth={0.5} opacity={0.7} />
                          {/* Front opening (cut-out look) */}
                          <path d={frontPath} fill={COL.nicheBack} stroke={COL.frontStroke} strokeWidth={0.8} />
                          {/* Side walls — converging perspective */}
                          <polygon points={`${fx1},${fArchTop + fr} ${fx1},${fyBase} ${bx1},${byBase} ${bx1},${bArchTop + br}`} fill="url(#wallGradL)" stroke={COL.frontStroke} strokeWidth={0.4} />
                          <polygon points={`${fx2},${fArchTop + fr} ${fx2},${fyBase} ${bx2},${byBase} ${bx2},${bArchTop + br}`} fill="url(#wallGradR)" stroke={COL.frontStroke} strokeWidth={0.4} />
                          {/* Ceiling (curved perspective panel where the spot sits) */}
                          <path d={ceilPath3D} fill="url(#ceilGrad3D)" stroke={COL.frontStroke} strokeWidth={0.5} />
                          {/* Light glow on ceiling and spilling forward */}
                          <ellipse cx={spotCx} cy={spotCy + 4} rx={28} ry={10} fill="url(#spotGlow3D)" />
                          {/* Spot fixture — ellipse because viewed obliquely */}
                          <ellipse cx={spotCx} cy={spotCy} rx={9} ry={3.5} fill="#1A1A1A" stroke={COL.frontStroke} strokeWidth={0.5} />
                          <ellipse cx={spotCx} cy={spotCy} rx={6.5} ry={2.4} fill="hsl(45 95% 78%)" />
                          <ellipse cx={spotCx} cy={spotCy} rx={3.5} ry={1.3} fill="hsl(45 100% 92%)" />
                          {/* Light cone projecting downward/forward */}
                          <polygon points={`${spotCx - 8},${spotCy + 2} ${spotCx + 8},${spotCy + 2} ${spotCx + 26},${fyBase - 4} ${spotCx - 26},${fyBase - 4}`} fill="hsl(45 100% 80%)" opacity={0.12} />
                          <polygon points={`${spotCx - 5},${spotCy + 2} ${spotCx + 5},${spotCy + 2} ${spotCx + 16},${fyBase - 4} ${spotCx - 16},${fyBase - 4}`} fill="hsl(45 100% 88%)" opacity={0.18} />
                          <text x={85} y={134} textAnchor="middle" fontSize={7.5} fill="hsl(var(--muted-foreground))">Ø{LIGHT_DIAMETER * 10}mm · gecentreerd in boog</text>
                        </g>
                      );
                    })()}
                  </svg>
